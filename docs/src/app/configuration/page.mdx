...
The nested `goal` object keeps goal provider and model settings separate from the browser `provider` and chat `model` keys. Use the full credential-bearing shape below only in a protected user or explicit config; project config should contain only nonsecret fields:
...
<table>
  <thead>
    <tr><th>Goal Config Key</th><th>Precedence (Highest to Lowest)</th><th>Default</th></tr>
  </thead>
  <tbody>
    <tr><td><code>goal.provider</code></td><td><code>AGENT_BROWSER_GOAL_PROVIDER</code> → <code>goal.provider</code></td><td><code>vercel</code></td></tr>
    <tr><td><code>goal.evalModel</code></td><td><code>--eval-model</code> → <code>AGENT_BROWSER_GOAL_MODEL</code> → <code>goal.evalModel</code></td><td>Selected provider default</td></tr>
    <tr><td><code>goal.textModel</code></td><td><code>--text-model</code> → <code>AGENT_BROWSER_GOAL_TEXT_MODEL</code> → <code>goal.textModel</code></td><td>Selected provider default</td></tr>
    <tr><td><code>goal.cloudflare.accountId</code></td><td><code>CLOUDFLARE_ACCOUNT_ID</code> → <code>goal.cloudflare.accountId</code></td><td>(required for Cloudflare)</td></tr>
    <tr><td><code>goal.cloudflare.apiToken</code></td><td><code>CLOUDFLARE_API_TOKEN</code> → <code>goal.cloudflare.apiToken</code></td><td>(required for Cloudflare)</td></tr>
    <tr><td><code>goal.cloudflare.gatewayId</code></td><td><code>CLOUDFLARE_AI_GATEWAY_ID</code> → <code>goal.cloudflare.gatewayId</code></td><td><code>default</code></td></tr>
    <tr><td><code>goal.vercel.apiKey</code></td><td><code>AI_GATEWAY_API_KEY</code> → <code>goal.vercel.apiKey</code></td><td>(required for Vercel goal mode)</td></tr>
  </tbody>
</table>

User and project goal settings merge field by field, including partial nested provider objects. An explicit `--config` or `AGENT_BROWSER_CONFIG` file replaces automatic user and project discovery. Model precedence is command-local option, nonempty environment variable, merged config, then selected-provider default. Provider and credential environment variables override config. Selecting a different provider does not reset an explicit evaluation or text model ID; override those IDs too, or omit model settings to use the selected provider's defaults. `AI_GATEWAY_URL` remains environment-only and this goal block does not configure `chat`.

Goal API keys and tokens in JSON are plaintext. Prefer `~/.agent-browser/config.json`, protect it with `chmod 600 ~/.agent-browser/config.json`, and never commit credentials in a project `agent-browser.json`. The CLI does not automatically load `.env` files and does not write goal credentials or defaults back to config.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>goal</code></td><td>Goal-specific flags and environment variables</td><td>nested provider, model, and credential object</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AI_GATEWAY_API_KEY</code></td><td>API key for Vercel AI Gateway. Required for AI chat; overrides <code>goal.vercel.apiKey</code> for Vercel goal mode.</td><td>(none)</td></tr>
    ...
    <tr><td><code>AGENT_BROWSER_GOAL_PROVIDER</code></td><td>Provider for <code>goal</code>; overrides <code>goal.provider</code>. Values are <code>vercel</code> or <code>cloudflare</code>.</td><td><code>vercel</code></td></tr>
    <tr><td><code>AGENT_BROWSER_GOAL_MODEL</code></td><td>Evaluation model for <code>goal</code>; overrides <code>goal.evalModel</code>.</td><td>Provider default: <code>typesafe-ai/jev</code> or <code>typesafe/jev</code></td></tr>
    <tr><td><code>AGENT_BROWSER_GOAL_TEXT_MODEL</code></td><td>Text model for <code>TYPE_TEXT</code>; overrides <code>goal.textModel</code>.</td><td>Provider default: <code>inception/mercury-2.5</code> or <code>@cf/qwen/qwen3-30b-a3b-fp8</code></td></tr>
    <tr><td><code>CLOUDFLARE_ACCOUNT_ID</code></td><td>Cloudflare account ID; overrides <code>goal.cloudflare.accountId</code>.</td><td>(none)</td></tr>
    <tr><td><code>CLOUDFLARE_API_TOKEN</code></td><td>Cloudflare API token with Account &gt; Workers AI &gt; Read permission; overrides <code>goal.cloudflare.apiToken</code>.</td><td>(none)</td></tr>
    <tr><td><code>CLOUDFLARE_AI_GATEWAY_ID</code></td><td>AI Gateway ID; overrides <code>goal.cloudflare.gatewayId</code>.</td><td><code>default</code></td></tr>
  </tbody>
</table>
