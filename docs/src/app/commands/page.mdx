...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Evaluation model, or <code>AGENT_BROWSER_GOAL_MODEL</code> (default: <code>typesafe-ai/jev</code>)</td>
    </tr>
    <tr>
      <td><code>--text-model &lt;model&gt;</code></td>
      <td>Text model for <code>TYPE_TEXT</code>, or <code>AGENT_BROWSER_GOAL_TEXT_MODEL</code> (default: <code>inception/mercury-2.5</code>)</td>
    </tr>
  </tbody>
</table>

`--debug` additionally writes every model request and reply to stderr, which is the quickest way to see what the model was offered on a step that went wrong. The `--json` result carries `status` (`done`, `blocked`, `denied`, `timeout`, `confirmation_required`, or `error`), the final `url`, `elapsedMs`, pending confirmation fields when applicable, and one entry per executed step with the operation, the target ref, role and name, any typed text, the target probability, the model's confidence, the operation distribution, and per-phase timings.
