...
agent-browser goal "<goal>"           # Evaluation model drives the open page toward one goal
```
...
Reference clicks account for iframe offsets and transforms, including cross-origin frames. Clicks fail before dispatch when another element covers the target's click point. The error names the covering element, for example `covered by <div#consent-banner>`. Dismiss or interact with that element, take a fresh snapshot, then retry the original action.

If a daemon reply is lost after a request may have been sent, the CLI and MCP report `Command outcome uncertain`. The command may already have executed, so mutating actions, confirmations, launches, and batches are not automatically replayed. Inspect the browser before deciding whether to repeat the action. A connection or write failure known to transfer zero request bytes can still recover automatically; any positive write is treated conservatively. Ordinary reads may launch or restore a browser; only a read guarded against those lifecycle changes qualifies for post-dispatch transport retry. Goal stops on an uncertain action outcome without treating it as a stale element.
...
Goal records a verified local component visual hit, such as a switch track or ripple surface, and accepts only that same hit at guarded dispatch, including after confirmation. A newly covering sibling is withheld. An existing dialog owning a target through an open or closed shadow slot is not a new useful interface. After `WAIT`, a fresh observation that lacks the old ref may supply replacement choices without a second full read; fingerprint change alone does not retire it. A native policy denial stops with its reason, while an operational read failure may be recovered by a later `WAIT`.

After a successful click, goal checks the exact snapshot node without scrolling, focusing, or matching a replacement by name. If a spinner or overlay covers it, goal polls within a fixed 1500 ms local cap and the overall deadline; late coverage uses only the cap's remaining time. Two Ready samples can proceed with unchanged snapshot text. At the cap, a covered or unknown target is withheld from `CLICK` choices; other controls and `WAIT` remain available. A later `WAIT` can reveal readiness without restarting the cap, and an unavailable node is reobserved before retirement or withholding. If that fresh read cannot finish within the original click cap, goal offers no old element indices; `WAIT` can request a fresh observation later, while unresolved completion is blocked. Spinner text and unrelated counters do not stop polling. Text-only and unstructured one-button overlays do not qualify as useful dialogs. If the actual covering element contains a visible dialog that does not enclose the clicked target, has an enabled control, and has no visible busy or progress region, and the page changes, the trigger stays withheld while a fresh model decision can say `DONE`; the changed content alone does not complete the goal. Delayed observed changes count as progress for the corresponding click. If the page changes while the model evaluates a guarded repeat or terminal decision while the click guard is active, goal discards the old decision. A guarded native click may scroll the exact node within its deadline, then checks its current document, geometry, and coverage after any confirmation. A lost reply or late input acknowledgement leaves a possibly dispatched click uncertain, without automatic replay. Unknown checks and cross-process iframe ancestor coverage are not treated as ready. A verified local component visual inside an existing target-owning dialog remains eligible only while the same node stays at the click point. A new or different sibling cover, including busy or progress content, is rejected during guarded dispatch and after approval; ordinary reference clicks retain their component visual allowance. This checks actionability now, not whether the application's transition finished. The private read-only probe has no standalone CLI or MCP tool: snapshot policy governs it, while guarded clicks retain click policy.

Give the open page one natural-language goal and let an evaluation model choose every action. On each step the `goal` command sends bounded text from the full accessibility snapshot plus a numbered element table to the model, which answers two typed questions in one request: the next operation and the element index it targets. Alert, status, log, and paragraph text is prioritized, remaining text is sampled from both ends, and actionable labels already present in the element table are not duplicated. Only observed elements are offered, so the model never produces a selector, a URL, or a script. When the operation is `TYPE_TEXT`, a small text model writes the field value from the goal.

Vercel AI Gateway is the default goal provider. Set `AGENT_BROWSER_GOAL_PROVIDER=cloudflare` to use Cloudflare instead. Environment-only configuration remains supported:
...
The Cloudflare token needs Account > Workers AI > Read permission; an AI Gateway-only token is not sufficient. The CLI does not load `.env` automatically. Export variables directly, or load a trusted file into the current shell with `set -a; source .env; set +a`. Cloudflare goal credentials are never sent to the Vercel gateway, and selecting Cloudflare for `goal` does not change `chat`.

Goal settings can instead use existing config discovery. Use this full credential-bearing shape only in a protected `~/.agent-browser/config.json` or explicit config; a project `./agent-browser.json` should contain only nonsecret fields:
...
JSON credentials are plaintext. Prefer the user config with `chmod 600 ~/.agent-browser/config.json` and never commit tokens in a project config. User and project goal settings merge field by field, including partial `cloudflare` and `vercel` objects. An explicit config replaces automatic discovery. Provider and credential environment variables override config. Model precedence is command-local option, nonempty environment variable, merged config, then selected-provider default. Selecting a different provider does not reset an explicit evaluation or text model ID; override those IDs too, or omit model settings to use the selected provider's defaults. `AI_GATEWAY_URL` remains environment-only. Goal config does not affect `chat`.
...
agent-browser goal --max-steps 10 "Open the pricing page"    # Cap the action budget
agent-browser -v goal "Accept the cookie banner"             # Probability, confidence, and page change per step
agent-browser -q goal "Accept the cookie banner"             # Only the final result
agent-browser --json goal "Accept the cookie banner"         # Structured output with every step and timing
```

Actions run through the normal command pipeline as `click @eN`, `fill @eN`, `scroll`, and `wait`, so action policies, `--confirm-actions`, `--allowed-domains`, and session isolation apply unchanged. Without `--confirm-interactive`, a pending action stops the run with `status: "confirmation_required"` plus its confirmation ID. With that flag, a TTY uses the normal prompt; non-TTY stdin retains the CLI's auto-denial and goal returns `status: "denied"`. Private probes use a 500 ms execution slice and observations or guarded clicks use at most 30 s per execution; human approval may use the remaining goal time or original click cap, then approved work receives a fresh bounded execution slice and a new exact-node check. Approval after that deadline sends bounded denial cleanup and never dispatches or records the pending action. Explicit denial after the deadline uses the same bounded cleanup and returns `denied`. The run stops on `DONE`, `BLOCKED`, the step budget, the time budget, or three consecutive actions without DOM, URL, or measured scroll progress. One terminal assessment is allowed after the last action in the budget, but no additional action can run. The exit code is `0` only on `DONE`. `DONE` is the model's opinion, not proof, so verify the outcome with `snapshot`, `get url`, or a screenshot. Start from a page that is already open; `goal` never launches a browser or navigates on its own. Snapshots can include same-process iframe and open shadow targets; guarded goal clicks conservatively reject unsupported cross-process iframe coverage. Canvas remains outside the snapshot.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Action budget (default: 40)</td>
    </tr>
    <tr>
      <td><code>--timeout &lt;ms&gt;</code></td>
      <td>Time budget in milliseconds (default: 120000), including the final URL read of up to 1000 ms within the remaining budget</td>
    </tr>
    <tr>
      <td><code>--eval-model &lt;model&gt;</code></td>
      <td>Evaluation model; overrides <code>AGENT_BROWSER_GOAL_MODEL</code>, then <code>goal.evalModel</code> config (Vercel default: <code>typesafe-ai/jev</code>; Cloudflare default: <code>typesafe/jev</code>)</td>
    </tr>
    <tr>
      <td><code>--text-model &lt;model&gt;</code></td>
      <td>Text model for <code>TYPE_TEXT</code>; overrides <code>AGENT_BROWSER_GOAL_TEXT_MODEL</code>, then <code>goal.textModel</code> config (Vercel default: <code>inception/mercury-2.5</code>; Cloudflare default: <code>@cf/qwen/qwen3-30b-a3b-fp8</code>)</td>
    </tr>
  </tbody>
</table>

`--debug` additionally writes every model request and reply to stderr, which is the quickest way to see what the model was offered on a step that went wrong. The `--json` result carries the selected `provider`, `model`, and `textModel`, plus `status` (`done`, `blocked`, `denied`, `timeout`, `confirmation_required`, or `error`), the final `url`, `elapsedMs`, pending confirmation fields when applicable, and one entry per executed step with the operation, the target ref, role and name, any typed text, the target probability, the model's confidence, the operation distribution, and per-phase timings.

On `DONE`, the final `url` is refreshed through one `get url` read, bounded to 1000 ms or the remaining goal time budget, whichever is smaller. If the read fails, times out, or returns no usable URL, the result keeps `status: "done"` and the last observed URL. A missing live browser/page, an oversized response, or a launch/relaunch lifecycle result also falls back. The read never launches or replaces a browser, creates a tab, retries, respawns the daemon, or prompts. Step URLs remain the observations recorded at those steps. This read does not wait for navigation to finish; a route change can still occur afterward. The MCP `agent_browser_goal` tool returns the same result.
