...
Actions run through the normal command pipeline as `click @eN`, `fill @eN`, `scroll`, and `wait`, so action policies, `--confirm-actions`, `--allowed-domains`, and session isolation apply unchanged. Without `--confirm-interactive`, a pending action stops the run with `status: "confirmation_required"` plus its confirmation ID. With that flag, a TTY uses the normal prompt; non-TTY stdin retains the CLI's auto-denial and goal returns `status: "denied"`. A denial or an approval received after the deadline never dispatches or records the pending action. The run stops on `DONE`, `BLOCKED`, the step budget, the time budget, or three consecutive actions without DOM, URL, or measured scroll progress. One terminal assessment is allowed after the last action in the budget, but no additional action can run. The exit code is `0` only on `DONE`. `DONE` is the model's opinion, not proof, so verify the outcome with `snapshot`, `get url`, or a screenshot. Start from a page that is already open; `goal` does not navigate on its own, and it cannot see inside iframes, shadow roots, or canvas.
