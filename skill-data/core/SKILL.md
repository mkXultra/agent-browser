...
Goal retains a verified local component visual hit for a guarded control, including after approval, and rejects a newly covering sibling hit. An existing dialog that owns a target through an open or closed shadow slot is not treated as a newly useful covering interface. After `WAIT`, a fresh observation missing the original ref can supply replacement controls without a second full read; fingerprint change alone does not retire that ref. Policy-denied reads stop with the policy reason, while operational failures can remain recoverable through `WAIT`.

After a successful click, goal checks the same snapshot node for coverage without scrolling, focusing, or matching a replacement by name. It polls covered targets for up to 1500 ms within the overall timeout; coverage that begins later uses only the remaining original cap. Two Ready samples can proceed with unchanged snapshot text; a covered or unknown target at the cap is withheld from `CLICK` choices while other controls and `WAIT` remain available. A later `WAIT` can reveal readiness without resetting the cap. An unavailable node is reobserved before retirement or withholding so the model can assess the new page. If that fresh read cannot finish within the original click cap, goal offers no old element indices; `WAIT` can request a fresh observation later, while unresolved completion is blocked. Spinner text and unrelated counters do not end polling. Text-only and unstructured one-button overlays do not qualify as useful dialogs. A visible dialog attached to the actual covering element but not enclosing the clicked target, with an enabled control and no visible busy or progress region, together with changed page content, keeps the trigger withheld but permits a fresh model `DONE` assessment; content change alone is not completion. Delayed observed changes count as action progress. Goal reobserves a guarded repeat or terminal decision while the click guard is active when the model may have used an older page. At dispatch, goal may scroll the exact node within its deadline and then rechecks document identity and coverage, including after confirmation. A possibly dispatched click with a lost or late acknowledgement has an uncertain outcome and is never replayed automatically. Unknown checks and cross-process iframe ancestor coverage are not ready. A verified local component visual inside an existing target-owning dialog remains eligible only while the same node stays at the click point. A new or different sibling cover, including busy or progress content, is rejected during guarded dispatch and after approval; ordinary reference clicks retain their component visual allowance. Readiness is actionability now, not proof that the application finished updating. The private probe has no standalone CLI or MCP tool because it only serves goal; snapshot policy governs the probe, and normal click policy governs dispatch. Interactive approval uses the applicable remaining goal or original click cap; approved work gets a fresh bounded execution slice within that allowance and repeats the exact-node check; late approval or explicit denial uses bounded nonexecuting cleanup.
...
Vercel is the default goal provider and needs `AI_GATEWAY_API_KEY`. Set `AGENT_BROWSER_GOAL_PROVIDER=cloudflare` to use Cloudflare instead; that needs `CLOUDFLARE_ACCOUNT_ID` and `CLOUDFLARE_API_TOKEN` with Workers AI Read permission. Goal provider, models, and credentials may instead use the nested `goal` object in normal user, project, or explicit config discovery. Provider and credential environment variables override config. Model precedence is command-local option, nonempty environment variable, merged config, then selected-provider default. Selecting a different provider does not reset an explicit evaluation or text model ID; override those IDs too, or omit model settings to use the selected provider's defaults. JSON credentials are plaintext, so prefer `~/.agent-browser/config.json` with mode 600 and never commit tokens in `agent-browser.json`. The CLI does not load `.env`, so export variables or explicitly source a trusted file first. Goal config is separate from chat configuration. On every step the model sees bounded text from the full accessibility snapshot plus a numbered element table and returns an operation (`CLICK`, `TYPE_TEXT`, `SCROLL_UP`, `SCROLL_DOWN`, `WAIT`, `DONE`, `BLOCKED`) and an element index; the CLI maps the index back to a `@ref` and runs the normal command, so policies and confirmations still apply. Diagnostic and paragraph text is prioritized within the bound, and actionable labels already in the table are not duplicated. A pending confirmation is not an executed step. Without `--confirm-interactive`, the run stops with `confirmation_required` and a confirmation ID for `confirm` or `deny`. With that flag, use a terminal for the prompt; non-TTY stdin auto-denies and goal returns `denied`. An approval received after the goal deadline triggers denial cleanup and returns `timeout` without dispatching the action. For ordinary commands approved with `--confirm-interactive`, output and exit status come from the executed inner command rather than the confirmation envelope. The model may assess `DONE` after the last allowed action but cannot exceed the action budget. Say in the goal when to stop. Exit code `0` means the model said `DONE`, which is its opinion: verify with `snapshot -i`, `get url`, or a screenshot before you report success. Open the target page first; goal never launches a browser or navigates on its own. Log in first with `auth login`; never put a password in a goal. Snapshots can include same-process iframe and open shadow targets, but guarded goal clicks conservatively reject unsupported cross-process iframe coverage. Canvas remains outside the snapshot. See `references/commands.md` for provider defaults, config shape, and overrides.

On `DONE`, the final URL is refreshed once with `get url`, within 1000 ms and the remaining goal time budget. If no usable URL is returned, including an oversized response or a missing live browser, completion stays successful with the last observed URL. Both the daemon reply and, with WebDriver, the entire backend HTTP response are capped at 64 KiB. The WebDriver completion read also has a 1000 ms deadline covering response reads and bounded parsing, then releases the daemon for subsequent commands. The read never launches or replaces a browser, creates a tab, retries, respawns the daemon, or prompts. Step URLs remain historical observations, and the final read does not wait for navigation to finish. Standalone `get url` behavior is unchanged.
