...
For a component control, goal records a verified local visual hit and permits only that same visual to receive a guarded click, including after confirmation. A newly covering sibling remains blocked. A target assigned into its existing dialog through either an open or closed shadow slot does not make that dialog a separate useful interface. A fresh post-`WAIT` observation that lacks the old ref supplies replacement choices without another full read; fingerprint change alone is insufficient. Native policy denial ends the run with its reason; operational read failure may be recovered by a later `WAIT`.

After each successful click, goal probes the exact snapshot node without scrolling, focusing, or matching a replacement by name. A covered node is polled within a fixed 1500 ms local cap and the overall goal deadline; late coverage uses only the original cap's remainder. Two Ready samples can proceed even if the snapshot is unchanged. At the cap, a covered or unknown node is withheld from `CLICK` choices with a concise reason, while other controls and `WAIT` remain available. A later `WAIT` can reveal readiness without renewing the cap; an unavailable node is reobserved before retirement or withholding so the new page can be assessed. If that fresh read cannot finish within the original click cap, goal offers no old element indices; `WAIT` can request a fresh observation later, while unresolved completion is blocked. Spinner text and unrelated counters do not end polling. Text-only and unstructured one-button overlays do not qualify as useful dialogs. A visible dialog attached to the actual covering element but not enclosing the clicked target, with an enabled control and no visible busy or progress region plus changed page content may let a fresh model decision say `DONE` while its trigger stays covered; content change alone does not complete the goal. Delayed observed changes count as progress for the corresponding click. A change during evaluation of a guarded repeat or terminal decision while the click guard is active discards that decision. Guarded click dispatch may scroll the exact node within its deadline, then checks current document, geometry, and coverage after any confirmation. A possibly dispatched click with a lost or late reply is uncertain and cannot be replayed automatically. Probe failure or unsupported cross-process iframe ancestor coverage is Unknown, never Ready. A verified local component visual inside an existing target-owning dialog remains eligible only while the same node stays at the click point. A new or different sibling cover, including busy or progress content, is rejected during guarded dispatch and after approval; ordinary reference clicks retain their component visual allowance. These checks establish current actionability, not completion of background application work. The read-only probe is private to goal, has no separate CLI or MCP tool, and uses snapshot policy; guarded dispatch uses ordinary click policy. Human approval uses the remaining goal or click-local deadline; approved work receives a fresh bounded execution slice within that allowance, followed by another exact-node check; late approval or explicit denial sends bounded nonexecuting cleanup. Goal requires a page already open in the session and never launches a browser.
...
AGENT_BROWSER_GOAL_PROVIDER=cloudflare agent-browser goal "<goal>" # Cloudflare Jev and Workers AI defaults
...
On `DONE`, the final `url` uses one fresh `get url` read through the normal command pipeline, with a limit of 1000 ms or the remaining goal time budget, whichever is smaller. If it fails, times out, or returns no usable URL, completion stays successful with the last observed URL. This includes a missing live browser/page, oversized responses, and launch/relaunch lifecycle results. The read never launches or replaces a browser, creates a tab, retries, respawns the daemon, or prompts. Step URLs remain their recorded observations. No navigation polling is performed, and a later route change can still occur after the read. The MCP `agent_browser_goal` tool preserves this behavior.
