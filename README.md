...
Goal preserves a verified component visual hit for a guarded control, such as a ripple surface or switch track, across readiness checks and approval. A different sibling hit is coverage, even within the same component. A pre-existing target-owning dialog cannot qualify as a newly useful covering interface when the target is assigned through an open or closed shadow slot. After `WAIT`, an observation that already lacks the old ref supplies replacement choices without a second full read; a changed page fingerprint alone does not retire the node. A policy-denied observation stops with its reason, while an operational read failure can remain recoverable through `WAIT`.

After a successful click, `goal` checks the clicked snapshot node without scrolling, focusing, or matching a replacement by name. If a spinner or overlay covers it, goal polls within a fixed 1500 ms cap and the overall timeout. A continuously Ready node can proceed even if the snapshot is unchanged. Coverage that begins later resumes polling only within the original cap. At the cap, a covered or unknown target is withheld from `CLICK` choices with a short explanation; other controls and `WAIT` remain available. Fresh readiness after a later `WAIT` can restore that target without restarting the cap. An unavailable node is reobserved before retirement or withholding, leaving the new page for another model decision without implying success. If that fresh read cannot finish within the original click cap, goal offers no old element indices; `WAIT` can request a fresh observation later, while unresolved completion is blocked. Spinner text and unrelated counters do not end polling. Text-only and unstructured one-button overlays do not qualify as useful dialogs. If the actual covering element contains a visible dialog that does not enclose the clicked target, has an enabled control, and has no visible busy or progress region, and the page changes, the trigger stays withheld, but the model can assess `DONE` from the fresh page; changed content alone does not complete the goal. Delayed observed page changes count as action progress. Goal discards a changed-page decision during an active guarded repeat or terminal assessment while the click guard is active. A native goal click may scroll the exact node within its deadline, then checks its current document, geometry, and coverage again, including after confirmation. A lost reply or late input acknowledgement can leave the click outcome uncertain and is never an automatic retry. Unknown probe results are never treated as ready; cross-process iframe ancestor coverage is currently unsupported. A verified local component visual inside an existing target-owning dialog remains eligible only while the same node stays at the click point. A new or different sibling cover, including busy or progress content, is rejected during guarded dispatch and after approval; ordinary reference clicks retain their component visual allowance. Readiness means a target is actionable now, not that the application finished updating. The internal probe has no standalone CLI or MCP tool because it is only used by goal; it runs under snapshot policy, while guarded clicks retain normal click policy and confirmation.
...
Every action runs through the normal command pipeline (`click @eN`, `fill @eN`, `scroll`, `wait`), so action policies, `--confirm-actions`, `--allowed-domains`, and session isolation apply unchanged. Without `--confirm-interactive`, a pending action stops the run with `status: "confirmation_required"` and actionable confirmation data. With that flag, a TTY uses the normal prompt; non-TTY stdin retains the CLI's documented auto-denial and goal returns `status: "denied"`. Private probes use a 500 ms execution slice and observations or guarded clicks use at most 30 s per execution; human approval uses the remaining goal budget or original click cap, then approved work gets a fresh bounded execution slice and rechecks the exact node. Approval after that deadline sends bounded denial cleanup, returns `timeout`, and never dispatches or records the pending action. An explicit denial after the deadline uses the same bounded cleanup and returns `denied`. The run stops on `DONE`, `BLOCKED`, the step budget, the time budget, or three consecutive actions without DOM, URL, or measured scroll progress, and exits `0` only on `DONE`. The model gets one terminal assessment after the last allowed action but cannot execute another action. The time budget bounds private goal reads and guarded clicks, including daemon queueing and browser input acknowledgements; a possibly dispatched click that times out has an uncertain outcome. `DONE` is the model's opinion, not proof: verify the outcome with `snapshot`, `get url`, or a screenshot. Start from a page that is already open; `goal` never launches a browser or navigates on its own. Snapshots can include same-process iframe and open shadow targets; guarded goal clicks conservatively reject unsupported cross-process iframe coverage. Canvas remains outside the snapshot.
